...
const COMPETENCIES = [
  {
    ...
    desc: 'React Native & Expo delivering pixel-perfect UIs across iOS and Android from a single codebase.',
  },
  {
    icon: '⚡',
    title: 'Performance Optimization',
    desc: '35% load time reduction, 25% memory improvement, 30% smaller bundles — measurable, not theoretical.',
  },
  {
    icon: '🏗',
  ...
  },
  {
    ...
    desc: 'Figma to App Store — EAS builds, CI/CD pipelines, and predictable multi-platform release cycles.',
  },
];
...
const EXPERIENCE = [
  {
    ...
    points: [
      ...
      'Improved app performance by 40% through profiling, lazy loading, and async optimization strategies.',
      'Maintained 99.2% crash-free rate and reduced bundle size by 30% across 50+ successful deployments.',
  ...
  },
  {
    ...
    current: false,
    points: [
      'Led core mobile features serving 50K+ users, increasing engagement by 45%.',
      'Achieved 35% load time reduction and 25% memory footprint decrease through performance optimization.',
      'Shipped to iOS App Store and Google Play with consistent 4.5+ star ratings.',
  ...
  },
  {
    ...
    points: [
      ...
      'Integrated REST APIs for auth, data fetching, and real-time updates in an Agile team.',
    ],
  },
];
...
const APPS = [
  {
    ...
    challenge: "Cambodia's dominant property audience expects speed, trust, and depth of listings in one app.",
  ...
  },
  {
    ...
    challenge: 'Classifieds across Laos need a localized, responsive experience without fragmenting the engineering playbook.',
  ...
  },
  {
    ...
    challenge: "PNG's leading portal must perform under the geographic and network conditions of regional expansion.",
  ...
  },
  {
    ...
    challenge: 'Real-estate teams lose deals when lead visibility and follow-up break down across tools.',
  ...
  },
  {
    ...
    challenge: 'Live shopping blends real-time media, commerce, and trust — small instability shows up as churn.',
  ...
  },
  {
    ...
    challenge: 'Consistent performance and 99%+ stability across a rapidly growing, diverse user base.',
    logo: '/assets/logos/lamudi.png',
    cover: '/assets/store-images/lamudi.png',
    ios: '#',
    android: 'https://play.google.com/store/apps/details?id=com.lamudiconnect',
  },
];

const SKILLS = {
  'Mobile Development': [
    'React Native', 'Expo', 'React Native CLI', 'Flutter',
    'Redux / FLUX', 'React Navigation', 'TypeScript',
    'Push Notifications (FCM/APNS)', 'Deep Linking',
    'Reanimated 2', 'App Store', 'Google Play', 'EAS / Fastlane',
  ],
  'Web & Tools': [
    'React & Next.js', 'JavaScript ES6+', 'REST APIs', 'GraphQL',
    'Firebase & Analytics', 'AsyncStorage', 'Jest & Testing Library',
    'Git & GitHub', 'CI/CD', 'HTML5 / CSS3',
  ],
  'Leadership & Process': [
    'Technical Roadmaps', 'System Design', 'API Alignment',
    'Performance Profiling', 'Code Review', 'Mentorship',
    'Agile / Scrum', 'Cross-team Collaboration', 'Release Management',
  ],
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll('.h-reveal').forEach((el) =>
    ...
    <div className="h-page">
      ...
      <nav className={`h-nav${navScrolled ? ' h-nav--solid' : ''}`}>
        <div className="h-nav__inner">
          ...
          <div className={`h-nav__links${menuOpen ? ' h-nav__links--open' : ''}`}>
            {navLinks.map((s) => (
              <button
                key={s}
                onClick={() => scrollTo(s)}
            ...
            <Link to="/resume" className="h-nav__resume">View Resume ↗</Link>
          </div>
          <button
      ...
      <section id="hero" className="h-hero">
        ...
        <div className="h-hero__body">
          <div className="h-hero__left">
            <span className="h-badge">Open to senior opportunities</span>
            <h1 className="h-hero__name">
              Kimhuo<br />
              <span className="h-hero__name-accent">Chhoeurng</span>
            </h1>
            ...
            <p className="h-hero__sub">
              I architect and ship cross-platform mobile experiences — from system design
              and API alignment to stable App Store and Google Play releases.
            </p>
            <div className="h-hero__cta">
              <button className="h-btn h-btn--primary" onClick={() => scrollTo('apps')}>
                View My Work
              </button>
              <button className="h-btn h-btn--ghost" onClick={() => scrollTo('about')}>
                About Me
              </button>
            ...
            <div className="h-hero__stats">
              {STATS.map((s) => (
                <div key={s.label} className="h-stat">
                  <span className="h-stat__val">{s.value}</span>
          ...
          <div className="h-hero__right">
            <div className="h-hero__photo-wrap">
              <div className="h-hero__photo-ring" aria-hidden="true" />
              <img
                src="/profile.jpg"
        ...
        <div className="h-hero__contact">
          ...
          <span className="h-hero__dot" />
          <a href="https://www.linkedin.com/in/kimhuo/" target="_blank" rel="noopener noreferrer">
      ...
      <section id="about" className="h-section">
        <div className="h-section__inner">
          ...
          <p className="h-about__body h-reveal">
            Over seven years, I have moved from hands-on React Native engineering to leading
            mobile delivery for market-defining products. Early on, the focus was execution
            — clean components, solid state patterns, reliable integrations. That foundation
            expanded into strategy: setting technical direction, keeping codebases healthy
            across React Native CLI and Expo, and making sure mobile and backend stay in
            lockstep so releases are predictable, not heroic. I still write and review every
            ...
            {EXPERIENCE.map((job, i) => (
              ...
              >
                <div className="h-job__line">
                  <div className={`h-job__dot${job.current ? ' h-job__dot--active' : ''}`} />
                </div>
                <div className="h-job__content">
                  <div className="h-job__top">
                    <div>
                      <h3 className="h-job__company">{job.company}</h3>
                      <p className="h-job__role">{job.role}</p>
                    </div>
                    <div className="h-job__meta">
                      {job.current && <span className="h-job__badge">Current</span>}
                      <span className="h-job__period">{job.period}</span>
                      <span className="h-job__location">{job.location}</span>
                    </div>
                  </div>
            ...
            {APPS.map((app, i) => (
              ...
              >
                ...
                <div className="h-app__cover">
                  {app.cover ? (
                    <img src={app.cover} alt={`${app.name} screenshot`} />
                  ) : (
                ...
                <div className="h-app__header">
                  {app.logo && (
                    <img src={app.logo} alt={`${app.name} logo`} className="h-app__logo" />
                  )}
                  <div>
                    <h3 className="h-app__name">{app.name}</h3>
                    <span className="h-app__tagline">{app.tagline}</span>
                  </div>
                </div>

                <p className="h-app__desc">{app.desc}</p>
                <div className="h-app__challenge">
                  <span className="h-app__challenge-lbl">Challenge</span>
                  <p>{app.challenge}</p>
                </div>
                <div className="h-app__stores">
                  <a href={app.ios} target="_blank" rel="noopener noreferrer" className="h-store-btn">
                    <AppleIcon /> App Store
                  </a>
                  <a href={app.android} target="_blank" rel="noopener noreferrer" className="h-store-btn">
        ...
        <div className="h-section__inner">
          ...
          <div className="h-skills">
            {Object.entries(SKILLS).map(([cat, items], i) => (
              <div key={cat} className={`h-skills__col h-reveal h-reveal--d${i + 1}`}>
                <h3 className="h-skills__title">{cat}</h3>
                <div className="h-skills__tags">
                  {items.map((sk) => (
                    <span key={sk} className="h-tag">{sk}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      <footer className="h-footer">
        <div className="h-footer__inner">
          <p className="h-footer__logo">KH<span className="h-accent">.</span></p>
          <div className="h-footer__links">
            <a href="mailto:kimhuo99@gmail.com">kimhuo99@gmail.com</a>
            <a href="https://www.linkedin.com/in/kimhuo/" target="_blank" rel="noopener noreferrer">LinkedIn</a>
            <Link to="/resume">Resume</Link>
          </div>
          <p className="h-footer__copy">© 2025 Kimhuo Chhoeurng · Phnom Penh, Cambodia</p>
        </div>
      </footer>
